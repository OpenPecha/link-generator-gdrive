...
def find_google_drive_file_id(service, file_name, file_path):
    ...
    try:
        ...
        if len(files) == 1:
            ...
            print(f"Multiple files found for {file_name}, manual check required.")
        return None
...
def main():
    ...
    try:
        ...
        catalog_df = pd.read_csv('/Users/tenzingayche/Documents/LLM-corpus/summary.csv')

        for index, row in catalog_df.iterrows():
            google_drive_file_id = find_google_drive_file_id(service, row['Repository'], row['Repository'])
            if google_drive_file_id:
                link = generate_shareable_link(service, google_drive_file_id)
                if link:
                    catalog_df.at[index, 'google_drive_file_id'] = google_drive_file_id
                    catalog_df.at[index, 'shareable_link'] = link
                    catalog_df.to_csv('new catalog.csv', index=False)
                    print(f"Updated catalog saved. index: {index}")
